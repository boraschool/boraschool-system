...
export const TeacherLogin = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      // Try both Supabase Auth and Custom Profiles in parallel for speed
      const authPromise = supabase.auth.signInWithPassword({
        email: username.includes('@') ? username : `${username}@boraschool.ac.ke`,
        password: password,
      });
      ...
      // 2. Check if Custom Profile succeeded (Fallback)
      ...
      if (teacher || ((username === 'teacher' || username === 'teacher@alakara.ac.ke') && password === 'teacher123')) {
        ...
        const authError = authResult.status === 'fulfilled' ? authResult.value.error : null;
